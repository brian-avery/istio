		...
		case discReq, ok := <-reqChannel:
			...
			case RouteType:
				...
				if discReq.ErrorDetail != nil {
					...
					incrementXDSRejects(rdsReject, discReq.Node.Id, discReq.ErrorDetail.Message)
			...
			case VhdsType:
				...
				if discReq.ErrorDetail != nil {
					...
					incrementXDSRejects(vhdsReject, discReq.Node.Id, discReq.ErrorDetail.Message)
					totalXDSRejects.Increment()
					continue
				}
...
func splitVHost(entry string) (uint32, string, error) {
	...
	entryParts := strings.SplitN(entry, vhostSeparator, 2)
	if len(entryParts) != 2 {
